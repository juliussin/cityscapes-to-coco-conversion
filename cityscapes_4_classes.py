...
def convert_cityscapes_instance_only(data_dir, out_dir, verbose=False):
    ...
    json_name = 'cityscapes_4_classes_%s.json'
        ...
        for root, _, files in os.walk(os.path.join(data_dir, ann_dir)):
            for filename in files if verbose else tqdm(files, desc=os.path.split(root)[1]):
                if filename.endswith(polygon_json_file_ending):
                    ...
                    if len(images) % 50 == 0 and verbose:
...
if __name__ == '__main__':
    ...
    if args.dataset == "cityscapes":
        convert_cityscapes_instance_only(args.datadir, args.outdir, verbose=args.verbose)
    else:
